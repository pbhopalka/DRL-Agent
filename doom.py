    ...
    def __init__(self, ob_space, ac_space, scope, trainer):
        with tf.variable_scope(scope):
            self.inputs = tf.placeholder(
                shape=[None, ob_space], dtype=tf.float32)
        ...
        with sess.as_default(), sess.graph.as_default():
            while not coord.should_stop():
                ...
                while not done:
                    ...
                    #This is kind of using greedy approach. Not following epsila-greedy
                    new_observation, reward, done, _ = self.env.step(
...
load_model = False
model_path = './model'
s_size = 7056  #this means [1,84,84]
a_size = 6
...
for i in range(num_workers):
    ...
    workers.append(
        Worker(env, i, s_size, env.action_space.n, trainer, model_path, global_episodes))
